<template>
  <div class="container margin_60_35">
    <h1 class="mb-5">Treatment Center Profile</h1>
    <div class="row">
    ...
    <p class="text-right"><a @click="next" class="btn_1 rounded add_top_30" style="color: white;">Next</a></p>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    asyncData({ params, error, $axios }) {
      return $axios.get(`/facility/get/${params.id}`).then((res) => {
    ...
    methods: {
      ...
      save: function() {
        ...
        this.$axios.post(`/profile`, formData, { headers: { 'Content-Type': 'multipart/form-data'} }).then((res) => {
          if(res.data && !res.data.error){
            this.$store.commit('profile/setFacility', this.item);
      ...
      next: function() {
        ...
        this.$store.commit('profile/setFacility', this.item);
        this.$store.commit('profile/set', this.form);
        this.$store.commit('profile/setImages', this.images);
        this.$store.commit('profile/setLogo', this.logo);
