...
<script>
  ...
  export default {
    ...
    asyncData({ params, error, $axios }) {
      return $axios.get(`/portal/reviews/${params.id}`).then((res) => {
        var date = new Date();
        var years = [];
    ...
    methods: {
      ...
      remove: function() {
        swal({title:"Remove", text:"Are you sure", icon:"info", buttons:['No','Yes']}).then(function(val){
          ...
          this.$axios.delete(`/portal/reviews${this.params.id}`, request).then((res) => {
      ...
      save: function() {
        ...
        this.$axios.put(`/portal/reviews`, request).then((res) => {
          if(res.data && !res.data.error){
            swal({title:"Notice", text:"Review Updated Successfully", icon:"success"}).then(function(val){
            }.bind(this));
          } else {
      ...
      search: function() {
        ...
        this.$axios.get(`/portal/reviews/${this.$route.params.id}`, {params: request}).then((res) => {
