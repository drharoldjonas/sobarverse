<template>
  <div class="adspots margin_60_35">
    <div class="container">
      <h3 class="mb-5">Subscriptions</h3>
      <div class="form-group row">
        ...
        <div class="col-lg-6">
          ...
          <a class="btn_1" @click="addSubscription">New</a>
      ...
      <div class="table-responsive">
        <table class="table table-condensed">
          ...
          <tr v-for="(item, index) in items.data" :key="index">
            ...
            <td>{{ item.status }}</td>
            <td>{{ item.subscription_end | format_date }}</td>
            <td>{{ item.package ? item.package.price : 0 }}</td>
            <td>
              <input type="checkbox" @click="addToArray(item)" :checked ="isSelected(item)">
            </td>
            <td>
              <div style="display: flex;">
                <a class="btn_2" @click="edit(item)"><i class="fa fa-edit"></i>Edit</a>
      ...
      <!-- Modal -->
      <div class="modal fade in" id="subscriptionModal" tabindex="-1" role="dialog" aria-labelledby="subscriptionModalLabel" aria-hidden="true">
                  ...
                  <form id="form0">
                    <div class="col-md-12">
                      ...
                      <div class="form-group">
                        <div class="col-md-8 form-group">
                          <label>Package:</label>
                          <select class="selectpicker form-control" v-model="item.package_id">
                            <option value="">---Select A Package ---</option>
                            <option v-for="(plan, index) in plans" :value="plan._id" :key="index">{{ plan.name }}</option>
                          </select>
                        </div>
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        <div class="col-md-8 form-group">
                          <label>Expires:</label>
                          <client-only>
                            <datepicker v-model="item.subscription_end" input-class="form-control"></datepicker>
                          </client-only>
                        </div>
                      </div>
                    </div>
 ...
 <!-- Modal2 -->
      <div class="modal fade in" id="subscriptionModal2" tabindex="-1" role="dialog" aria-labelledby="subscriptionModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h2 class="modal-title" v-if="selected.length === 0">No items Select</h2>
              <h2 class="modal-title" v-else>Edit Subscription</h2>
            ...
            <div class="modal-footer">
              ...
              <button type="button" class="btn btn-primary" @click="save2">Save</button>
            </div>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    asyncData({ params, error, $axios }) {
      ...
      let URL1 = `/admin/subscriptions`;
      let URL2 = `/admin/packages`;
      ...
      return Promise.all(promiseArray).then((res) => {
        return { items: Object.assign({total:0, data:[]}, res[0].data), plans: res[1].data.data || []}
      }).catch((e) => {
        console.log(e)
      })
    },
    ...
    methods: {
      ...
      edit: function(item) {
        this.item = item;
        this.item.subscription_end = this.formatDate(this.item.subscription_end);
      ...
      changePage: function (n, obj) {
        ...
        edit2: function(item) {
        $('#subscriptionModal2').modal('show')
      },
      addSubscription2: function(item) {
        this.item = {};
        $('#subscriptionModal2').modal('show')
      },
      cancel2: function() {
        $('#subscriptionModal2').modal('hide')
      },
      save2: function(){
        this.cancel2();
        ...
        this.$axios[type](`/admin/subscriptions`, request).then((res) => {
      ...
      remove: function(obj){
        ...
        swal({title:"Remove", text:"Are you sure", icon:"info", buttons:['No','Yes']}).then(function(val){
          ...
          this.$axios.delete(`/admin/subscriptions/${obj._id}`, request).then((res) => {
            if(res.data && !res.data.error){
      ...
      search: function(o) {
        ...
        this.$axios.get(`/admin/subscriptions`, {params: request}).then((res) => {
