  ...
  <div class="container margin_60_35">
    <div class="wpb_text_column wpb_content_element ">
      <div class="wpb_wrapper">
        <h4 class="submit-form-user-info">Edit Vendor</h4>
      </div>
    </div>
    <form>
      <div class="row">
...
<!--              <textarea class="form-control" cols="20" v-model="facility.schoolInfo" maxlength="3000" rows=7></textarea>-->
<!--            </div>-->
            ...
            <div class="form-group col-md-12">
              <div class="row">
                <div class="col-md-6 required">
                  <label>Company</label>
                  <client-only>
                    <v-select label="name" :filterable="false" :options="options" v-model="facility.company" @search="onSearch">
...
<!--        </template>-->
        ...
        <div v-if="facility.featured" class="form-group col-md-12">
          <h5 class="formlabel">Select ad spot</h5>
          <div class="row">
            <template v-if="adspots.length">
              <div v-for="(adspot, index) in adspots" class="col-lg-4" :key="index">
                <label class="container_check">{{ adspot.name }} - {{ adspot.placement }}
                  <input type="radio" v-model="spot" :value="adspot._id">
                  <span class="checkmark"></span>
                </label>
              </div>
            </template>
            <ul v-else>
              <li>No Available Ad Spots</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    async asyncData({$axios, params}) {
      let [facility, db_vendor_categories] = await Promise.all([
        $axios.get(`/admin/facilities/${params.id}`),
        $axios.$post('/vendor-cat-populated')
      ]).catch((err) => {
        console.log(err)
      })

      return { facility: Object.assign({
          protocol: 'http://',
          primaryFocus: [],
          servicesProvided: [],
          careType: [],
        ...
        }, facility.data.data), db_vendor_categories: db_vendor_categories.data }
    },
    ...
    methods: {
      ...
      getSpots: function() {
        if(this.facility.featured) {
          this.$axios.get(`/admin/adspots/available`).then((res) => {
            this.adspots = res.data.data || [];
          })
        }
      },
      checkForm: function () {
      ...
      setProfile: function(data) {
        this.facility.profile = Object.assign(this.facility.profile, data)
      },
      ...
      setAdspot: function() {
        this.$axios.put(`/admin/adspots`, {id: this.spot, args: {facility_id: this.facility._id}}).then((res) => {
      ...
      save: function() {
        ...
        if(this.facility.company) this.facility.company_id = this.facility.company._id;
        else this.facility.company_id = '';
        formData.append('args', JSON.stringify(this.facility));

        this.$axios.put(`/admin/facilities`, formData, { headers: { 'Content-Type': 'multipart/form-data'} }).then((res) => {
          if(this.facility.featured && this.spot) this.setAdspot();
          if(res.data && !res.data.error){
            swal({title:"Notice", text:"Vendor Successfully Saved", icon:"success"}).then(function(val){
            }.bind(this));
          } else {
            //Handle errors
            console.log('eroor')
