      ...
      <div class="table-responsive">
        <table class="table table-bordered table-listing">
          ...
          <tr v-for="(item, index) in items.data" :key="index">
            <td>
              <nuxt-link class="btn_3" :to="{name: 'admin-reviews-id',  params: { id: item._id } }" target="_blank">Edit</nuxt-link>
            </td>
...
<script>
  ...
  export default {
    ...
    asyncData({ params, error, $axios }) {
      return $axios.get(`/admin/reviews`, {params: {max: 25}}).then((res) => {
    ...
    methods: {
      ...
      approve: function(item) {
        ...
        this.$axios.post(`/admin/reviews/approve`, request).then((res) => {
      ...
      remove: function(obj){
        ...
        swal({title:"Remove", text:"Are you sure", icon:"info", buttons:['No','Yes']}).then(function(val){
          ...
          this.$axios.delete(`/admin/reviews/${obj._id}`, request).then((res) => {
            if(res.data && !res.data.error){
      ...
      search: function(o) {
        ...
        this.$axios.get(`/admin/reviews`, {params: request}).then((res) => {
