...
<script>
  ...
  export default {
    ...
    asyncData({ query, params, error, $axios }) {
      return $axios.get(`/admin/transactions`, {params: {sSearch: query.name, max: 25}}).then((res) => {
    ...
    methods: {
      ...
      remove: function(obj){
        ...
        swal({title:"Remove", text:"Are you sure", icon:"info", buttons:['No','Yes']}).then(function(val){
          ...
          this.$axios.delete(`/adspots/transactions/${obj._id}`, request).then((res) => {
      ...
      search: function(o) {
        ...
        this.$axios.get(`/admin/transactions`, {params: request}).then((res) => {
